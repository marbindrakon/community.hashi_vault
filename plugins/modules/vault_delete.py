...
DOCUMENTATION = """
  ...
  attributes:
    check_mode:
      support: partial
      details:
        - In check mode, an success response will be returned and the delete will not be performed.
  seealso:
    - ref: community.hashi_vault.vault_delete lookup <ansible_collections.community.hashi_vault.vault_delete_lookup>
      description: The official documentation for the C(community.hashi_vault.vault_delete) lookup plugin.
...
EXAMPLES = """
- name: Remove an auth method mounted to auth/extra-jwt
...
def run_module():
    ...
    if hasattr(response, 'status_code'):
        if response.status_code in (200, 204):
            ...
            module.warn('Vault returned unexpected status code %i ' % response.status_code)
